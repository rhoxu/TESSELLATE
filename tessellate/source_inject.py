...
#     # flux_cadence = np.zeros(len(cadence_edges) - 1)
#     # for i in range(len(cadence_edges) - 1):
#     #     mask = (t_fine >= cadence_edges[i]) & (t_fine < cadence_edges[i+1])
#     #     flux_cadence[i] = np.trapz(flux_norm[mask], t_fine[mask]) if mask.sum() > 1 else 0
...
#     def __init__(self,sector,cam,ccd,n=8,job_output_path='.',working_path='.',data_path='/fred/oz335/TESSdata',prf_path='/fred/oz335/_local_TESS_PRFs'):
...
#         toward particular regimes by default.

#         valid_sites: array (n_sites, 2), (x, y) candidate positions
#         time_baseline: array of cadence timestamps IN DAYS (MJD)
#         cube_shape: (n_frames, ny, nx)
#         n_events: number of events to schedule
#         duration_range_min: (min, max) event duration IN MINUTES
#         duration_skew: (a, b) Beta params on normalized log-duration axis;
#                     a < b biases toward shorter events
#         K_range: (min, max) shape parameter range
#         K_skew: (a, b) Beta params on the positive-K portion, normalized to
#                 [0,1] then mapped to [0, K_range[1]]; a > b biases toward high K
#         p_negative_K: probability an event is drawn from the negative K range
#         duty_frac_range: (min, max) allowed duty_frac
...
#                 eventid, xcentroid, ycentroid, K, duty_frac, frame_start,
#                 frame_end, frame_duration, mjd_start, mjd_end, mjd_duration,
#                 overlap
#         """
...
#         def draw_snr():
#             return 10**(rng.uniform(0.45,2))    # draws between ~2.5 - 100
...
#             if not placed:
#                 print(f"Warning: event {eventid} could not be placed within "
...
#             t,flux = Gen_Event(source.K,np.nanmedian(np.diff(self.nav.time))*1440,source.mjd_duration*1440,source.duty_frac)
...
#             max_frame = frames[np.argmax(flux)]
...
#             # plt.figure()
#             # plt.imshow(cut,origin='lower',vmin=-noise,vmax=3*noise)
#             # print(noise)
...
#                 image *= peak_flux * f / np.nansum(image[1:4, 1:4])

#                 raw_cube[frames[j], yint-2:yint+3, xint-2:xint+3] += image

#         return raw_cube,injections

#     def apply_shifts(self,cube):
...
#             rawcube,injections = self.inject_sources(cut,n_events,
...
#             shifted_cube = self.apply_shifts(rawcube)
...
#                                 reduce=True,search=True,injection=True,plot=False,delete=False,injection=True,
